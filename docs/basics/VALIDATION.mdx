...
By default, Gleece does not validate response payloads against the response schema defined in the route. 
...
The response payload validation is **only** applied to routes with a defined response struct schema.

:::

:::warning

When response payload validation is enabled, Gleece does not return the actual validation error message to the client to avoid exposing server internals.
