...
While you can have multiple `@Query`, `@Path`, and `@Header` annotations, only one `@Body` annotation is allowed per route.

:::info

Parameter Types Support
- All primitive types (including pointers to them) are supported for any parameter source.
- Struct types are supported for a `@Body` parameter only.
