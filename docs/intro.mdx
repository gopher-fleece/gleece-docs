...
The `github.com/gopher-fleece/gleece/v2` package is only used during build time to generate the routers and the specification, and not needed to be added to server's `go.mod`. Run `go mod tidy` to clean up your dependencies graph.
...
See the full [configuration](/docs/basics/configuration.mdx) documentation for detailed explanation of all available properties.
